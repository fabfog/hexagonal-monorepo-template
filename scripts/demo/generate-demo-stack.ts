 ...
 *
 * Usage: pnpm exec tsx scripts/demo/generate-demo-stack.ts [--confirm-install] [--force]
...
import { toKebabCase } from "../../plop/lib/casing.ts";
import { runPnpmInstall } from "../../plop/lib/pnpm-install.ts";
import {
  DEMO_APPLICATION,
  DEMO_COMPOSITION,
...
function hasDemoPnpmInstallConsent(): boolean {
  return (
...
function runDemoPnpmInstall(stepNote: string): void {
...
function assertCleanOrForce(): void {
 ...
 */
function patchDemoStackDomainImports(): void {
  ...
  const useCaseEntityTypes: Record<string, string[]> = {
  ...
  const flowEntityTypes: Record<string, string[]> = {
...
function prependDomainEntityTypeImports(filePath: string, entityTypeNames: string[]): void {
...
function ensureDemoStackAppShell(): void {
...
function patchDemoWebRepositoryWiring(): void {
...
async function loadPlop() {
  ...
  const plopfile = path.join(REPO_ROOT, "plop", "plopfile.ts");
  const force = process.argv.includes("--force");
  return nodePlop(plopfile, { force, destBasePath: undefined });
}
...
async function main(): Promise<void> {
  ...
  for (const step of STEPS) {
    ...
    const { failures, changes } = await generator.runActions(answers);
