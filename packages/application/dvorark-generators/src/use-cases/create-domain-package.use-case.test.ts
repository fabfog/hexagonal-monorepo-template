...
import {
  CreateDomainPackageUseCase,
  DOMAIN_PACKAGE_GENERATOR_ID,
} from "./create-domain-package.use-case";

describe("CreateDomainPackageUseCase", () => {
  it("maps DTO to slug, renders templates, and writes files", async () => {
    ...
    expect(generatorBlueprintSource.load).toHaveBeenCalledWith(DOMAIN_PACKAGE_GENERATOR_ID);
    ...
    expect(workspaceWriter.writeFiles).toHaveBeenCalledWith("/tmp/ws", [
      {
        relativePath: "packages/domain/user-profile/package.json",
