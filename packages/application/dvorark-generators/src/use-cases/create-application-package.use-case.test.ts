...
import {
  ...
  CreateApplicationPackageUseCase,
} from "./create-application-package.use-case";

describe("CreateApplicationPackageUseCase", () => {
  it("maps DTO to slug, renders templates, and writes files under packages/application", async () => {
    ...
    const uc = new CreateApplicationPackageUseCase({
      templateRenderer,
      workspaceWriter,
    ...
    expect(generatorBlueprintSource.load).toHaveBeenCalledWith(APPLICATION_PACKAGE_GENERATOR_ID);
    ...
    expect(workspaceWriter.writeFiles).toHaveBeenCalledWith("/tmp/ws", [
      {
        relativePath: "packages/application/user-profile/package.json",
