...
module.exports = function registerApplicationAddDependencyToFlowGenerator(plop) {
  plop.setGenerator("application-add-dependency-to-flow", {
    ...
    prompts: [
      ...
      },
      {
        ...
        name: "flowName",
        message: "Select flow:",
        choices: (answers) => getApplicationFlowChoices(repoRoot, answers.packageName),
      },
      {
        type: "list",
      ...
      },
      {
        ...
        choices: (answers) => {
          const flowPascal = toPascalCase(answers.flowName);
          const depsInterfaceName = `${flowPascal}FlowDependencies`;
          const flowKebab = toKebabCase(answers.flowName);
          ...
          const { properties } = parseDependenciesInterface(flowSource, depsInterfaceName);
          ...
          if (!filtered.length) {
            throw new Error("All ports in this package are already present in the flow deps.");
          }
    ...
    actions: (data) => {
      const { packageName, flowName, portApplicationPackage, portFileName, portPropertyName } =
        data;
      const flowPascal = toPascalCase(flowName);
      const depsInterfaceName = `${flowPascal}FlowDependencies`;
      ...
      actions.push({
        type: "modify",
        path: `../packages/application/${packageName}/src/flows/${flowKebab}.flow.ts`,
        transform: (file) => {
          let updated = file;
