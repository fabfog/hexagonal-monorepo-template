 ...
 */
function parseParamNamesFromSignature(paramsStr) {
 ...
 */
function extractPromiseInnerType(returnType) {
 ...
 */
function parseImmerInteractionAdapterMeta(content) {
  ...
  return {
    ...
    stateInterfaceName: storeMatch[1],
  };
}
 ...
 */
function findExportedInterfaceBodyRange(source, interfaceName) {
 ...
 * @returns {string}
 */
function ensureCurrentInteractionOnState(source, stateInterfaceName) {
 ...
 */
function isAskMethod(name, askMethodNames) {
 ...
 * @param {Set<string> | string[]} askMethodNames
 */
function renderMethodImplementation(method, askMethodNames) {
...
function mergeImmerInteractionAdapterMethods(
  existing,
  { className, interfaceName, methods, askMethodNames }
) {
 ...
 */
function buildStateInterfaceDeclaration(stateInterfaceName, hasAnyAskMethod) {
  if (!hasAnyAskMethod) {
 ...
 *
 * @param {string} source
 * @param {{ stateInterfaceName: string, className: string, interfaceName: string, methods: { name: string, params: string, returnType: string }[], askMethodNames: Set<string> | string[] }} params
 */
function mergeImmerInteractionAdapterFile(source, params) {
