 ...
 */
function findInfrastructureProviderClass(sf: ts.SourceFile) {
 ...
 */
function findGetForContextMethod(classDecl: ts.ClassDeclaration) {
 ...
 */
function findReturnObjectLiteral(body: ts.Block) {
...
function assertNoConflictingMembers(
  members: ts.NodeArray<ts.ClassElement> | readonly ts.ClassElement[],
  memberNames: string[]
) {
 ...
 */
function assertNoReturnPropertyConflict(obj: ts.ObjectLiteralExpression, propName: string) {
...
/**
 * @param {CompositionInfraPrintContext} ctx
 * @returns {string}
 */
function printUpdatedCompositionInfrastructure(ctx: CompositionInfraPrintContext) {
  ...
  const transformer =
    (transformContext: ts.TransformationContext) => (sourceFile: ts.SourceFile) => {
  ...
  result.dispose();
  if (!transformed) {
    throw new Error("Internal error: composition infrastructure transform produced no output");
  }
