 ...
 */
function buildCompilerPathsForDomainPackage(repoRoot: string, domainPackageDir: string) {
 ...
 */
function findToSnapshotImplementation(classDecl: ts.ClassDeclaration) {
 ...
 */
function simplifyReadonlyObjectWrapper(typeStr: string) {
 ...
 * @param {string} typeStr
 */
function stripReadonlyPropertyModifiersFromDtoType(typeStr: string) {
  const protectedChunks: string[] = [];
  let s = typeStr.replace(/ReadonlyArray<|ReadonlyMap<|ReadonlySet</g, (match: string) => {
 ...
 * @param {string} typeStr
 */
function normalizeGetPropsDtoTypeString(typeStr: string) {
 ...
 * @param {string[]} lines
 */
function buildDtoInterfaceSource(entityPascal: string, lines: string[]) {
...
function generateApplicationEntityMapperSources(opts: GenerateApplicationEntityMapperSourcesOpts) {
  const { repoRoot, domainPackage, entityBasePascal } = opts;
  ...
  const dtoSource = buildDtoInterfaceSource(
    entityPascal,
    fields.map((f: MapperCodegenField) => f.dtoLine)
  );
