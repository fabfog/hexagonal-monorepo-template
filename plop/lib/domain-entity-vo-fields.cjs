 ...
 */
function listExportedVoClasses(repoRoot, domainPackage) {
 ...
 * @param {string} domainPackage
 */
function ensureDomainCoreDependency(repoRoot, domainPackage) {
 ...
 */
function getVoFieldChoices(repoRoot, entityDomainPackage) {
 ...
 */
function appendVoFieldToEntitySource(content, entityPascal, field) {
  ...
  const mergeNamed = (existing, additions) => {
    const set = new Set(
      existing
        .split(",")
        .map((s) => s.trim())
  ...
  if (field.source === "core") {
    ...
    if (m) {
      ...
      const insertAt = idm ? idm.index + idm[0].length : next.indexOf("\n") + 1;
