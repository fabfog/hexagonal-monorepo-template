...
- **Available generators** (match the names in the Plop menu):
  ...
  | **Composition**                              |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
  | `composition-package`                        | Create a new `@composition/<name>` shell: `package.json`, `tsconfig.json`, and `src/.gitkeep` only. Runtime roots (`src/isomorphic`, `src/server`, `src/client`) and matching `package.json` `exports` are added when you run `composition-feature-dependencies` for the runtimes you select.                                                                                                                                                                                                                                                                              |
  | `composition-feature-dependencies`           | Add a feature (e.g. DocumentEditor) to a composition package: for each selected runtime, ensures `src/<runtime>/` (`infrastructure.ts`, `index.ts`), merges `exports` (`"."`, `"./server"`, `"./client"` as needed), then adds `src/<runtime>/<feature-kebab>/dependencies.ts` and registers `createXxxDependencies` on `dependencies.<featureCamel>` (lazy getter + cache) in that runtime’s `index.ts`.                                                                                                                                                                  |
  ...
  | `composition-wire-infrastructure`            | Wire a `driven-*` package into composition: for each selected runtime, ensures `src/<runtime>/` if needed, merges `package.json` `exports`, then updates `src/<runtime>/infrastructure.ts` with lazy getters (`let _fooInstance` + `get foo()`), repository wiring (HTTP + DataLoader + correlation) when the driven package is `driven-repository-*`, and adds `@infrastructure/*` deps.                                                                                                                                                                                  |
  | **UI**                                       |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
